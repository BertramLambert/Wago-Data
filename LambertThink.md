...
- **邊緣排程自動化系統 (Edge-Schedule System)**:
  - **定時自動化**: 利用 Node-RED 處理時間邏輯（如 Cron Job），獨立於 PLC 主程式之外控制硬體輸出。
  - **營運靈活性**: 使用者可透過視覺化界面調整排程，無需重新燒錄 PLC 程式。
- **SQLite 資料記錄器 (SQLite DataLogger)**:
  - **本地持久化儲存**: 將關鍵運作數據存入內部 SQLite 資料庫，確保數據完整性與斷網追蹤。
- **通訊協議閘道器 (Protocol Gateway)**:
  - **IT/OT 技術融合**: 橋接工業協定（Modbus, OPC UA）與 IT 標準協定（MQTT, RESTful API, JSON）。
- **互動式儀表板 (Web HMI)**:
  - **即時監控**: 透過儀表與圖表將感測器數據與系統狀態視覺化。
  - **遠端控制**: 直接從網頁瀏覽器或行動裝置調整設定值。

---

## 🌐 邊緣伺服器叢集 (Edge Server Swarm)
*此部分展示如何將多台 WAGO 控制器整合為具備高可用性的地端邊緣伺服器。*

- **Docker Swarm 叢集管理**: 
  - **叢集建立**: 展示如何將多台控制器初始化並組合成 Manager 與 Worker 節點。
  - **服務編排 (Orchestration)**: 使用 Docker Stack 實現跨節點的自動化服務部署。
- **高可用性與容錯 (HA)**: 
  - 實驗當單一節點離線時，Container 服務自動轉移與復原的機制。
- **跨節點虛擬網路 (Overlay Network)**: 
  - 建立安全的內部網路，使不同節點間的數據交換不受實體網域限制。

---

## 🗺 未來展望 (Roadmap)

- [x] **單機控制與數據整合 (Done)**: 已完成 Codesys, Python 與 Node-RED 基本模組開發。
- [ ] **邊緣伺服器叢集 (In Progress)**: 實現多控制器的高可用性、負載平衡與分散式運算。
- [ ] **雲端與混合架構 (Planned)**: 
  - **第三方雲端同步**: 串接免費雲端服務 (如 Google Sheets, Adafruit IO) 進行遠端備份。
  - **私有雲監控中心**: 透過 VPN 技術整合多個遠端邊緣節點至自架雲端主機。

---

## 🏗 硬體控制器規格
- **CC100**: 751-9301 / 751-9401
- **BC100**: 750-8001
- **PFC200**: 750-8212 (Gen2)
